...
function mw_popup() {
    ...
    $sql = "select * from g4_popup where pp_use = '1' and '$g4[time_ymd]' between pp_start and pp_end order by pp_id desc";
    $qry = sql_query($sql);
		$ret.= "<script>bn_cookiedata = document.cookie; </script>";
    for ($i=0; $row=sql_fetch_array($qry); $i++) {
			...
			if ($row[pp_type] == "1") { // 레이어 팝업
					...
					$ret.= "<style type=\"text/css\">\n";
					$ret.= "#$popup_name-close { position:absolute; width:$row[pp_width]px; height:25px; left:-1px; top:0; margin:".($row[pp_height])."px 0 0 0; padding:0; }\n";
