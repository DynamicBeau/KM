...
<tr class=ht>
    <td>상품고유번호</td>
    <td>
        <input type=text name=it_codename value='<?=$it[it_codename]?>' style='width:97%;' required itemname='상품고유번호' class=ed>
    </td>
		<td>상품종류</td>
    <td>
        <input type=text name=it_kind value='<?=$it[it_kind]?>' style='width:97%;'  itemname='상품종류' class=ed>
    </td>
</tr>
...
<tr class=ht>
    ...
    <td colspan=3>
        ...
        <?=help("메인화면에 유형별로 출력할때 사용합니다.\n\n이곳에 체크하게되면 상품리스트에서 유형별로 정렬할때 체크된 상품이 가장 먼저 출력됩니다.");?><br>
				<input type=checkbox name="main_rolling" value='1' <?=($it[main_rolling] ? "checked" : "");?>>메인롤링에 표시
				<input type=checkbox name="mobile_intro" value='1' <?=($it[mobile_intro] ? "checked" : "");?>>모바일 제품소개에 표시
    </td>
</tr>
<tr class=ht>
    <td>제조사</td>
