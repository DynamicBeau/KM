...
<? if ($default[de_sms_use] == "xonda") { // 쏜다넷 사용 ?>
<!-- <form action="http://biz.xonda.net/biz/sms/process_F.asp" name="smsform" method=post autocomplete=off> -->
<form action="http://biz.xonda.net/biz/biz_newV2/SMSASP_WEBV4_s.asp" name="smsform" method=post autocomplete=off>
<input type="hidden" name="biz_id" value="<?=$default[de_xonda_id]?>">
<input type="hidden" name="smskey" value="<?=$default[de_xonda_smskey]?>">
<input type="hidden" name="return_url" value="<?="$g4[shop_admin_url]"?>/smssendresult.php">
<input type="hidden" name="receive_number" value="">
<input type="hidden" name="usrdata1" value="개별전송">
<? } else if ($default[de_sms_use] == "icode") { // 아이코드 사용 ?>
