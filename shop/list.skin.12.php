...
.itemlist td a img{margin:0 auto; border:1px solid #DDD;}
.itemlist td a:hover img{border:1px solid #AAA}
.itemlist .name_icon a{display:block;margin-top:6px; }
.itemlist .cust_amount{text-decoration: line-through}
.itemlist .amount{font-weight:bold; color:#F60}
.btn_zoom{margin-top:6px; cursor: pointer}
</style>
...
<table class="itemlist">
	...
	for ($i=0; $row=sql_fetch_array($result); $i++) {
		...
		if (file_exists("$g4[path]/data/item/{$row[it_id]}_l1")){ 
		echo "<p><img src='$g4[shop_img_path]/btn_zoom.gif'  onclick=\"javascript:win_open('./largeimage.php?it_id={$row[it_id]}', 'viewImage','left=50, top=50, width=600, height=600, scrollbars=no')\" class='btn_zoom'></p>";
		//echo "<p><img src='$g4[shop_img_path]/btn_zoom.gif'  onclick=\"javascript:popup_large_image('{$row[it_id]}', '{$row[it_id]}_l1', 600, 500, '$g4[shop_path]')\" class='btn_zoom'></p>";
		}
