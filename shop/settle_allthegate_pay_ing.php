...
/*공통사용*/
$AuthTy		= trim(iconv("utf-8", "euc-kr",$_POST["AuthTy"]));			//결제형태

$SubTy 		= trim(iconv("utf-8", "euc-kr",$_POST["SubTy"]));			//서브결제형태
$StoreId 	= trim(iconv("utf-8", "euc-kr",$_POST["StoreId"]));			//상점아이디
$OrdNo 		= trim(iconv("utf-8", "euc-kr",$_POST["OrdNo"]));			//주문번호
$Amt 		= trim(iconv("utf-8", "euc-kr",$_POST["Amt"]));					//금액
$UserEmail	= trim(iconv("utf-8", "euc-kr",$_POST["UserEmail"]));		//주문자이메일
$ProdNm 	= trim(iconv("utf-8", "euc-kr",$_POST["ProdNm"]));			//상품명

/*신용카드&가상계좌사용*/
$MallUrl 	= trim(iconv("utf-8", "euc-kr",$_POST["MallUrl"]));			//MallUrl(무통장입금) - 상점 도메인 가상계좌추가
$UserId 	= trim(iconv("utf-8", "euc-kr",$_POST["UserId"]));				//회원아이디


/*신용카드사용*/
$OrdNm 		= trim(iconv("utf-8", "euc-kr",$_POST["OrdNm"]));			//주문자명
$OrdPhone	= trim(iconv("utf-8", "euc-kr",$_POST["OrdPhone"]));			//주문자연락처
$OrdAddr 	= trim(iconv("utf-8", "euc-kr",$_POST["OrdAddr"]));				//주문자주소 가상계좌추가
$RcpNm 		= trim(iconv("utf-8", "euc-kr",$_POST["RcpNm"]));			//수신자명
$RcpPhone	= trim(iconv("utf-8", "euc-kr",$_POST["RcpPhone"]));		//수신자연락처
$DlvAddr	= trim(iconv("utf-8", "euc-kr",$_POST["DlvAddr"]));				//배송지주소
$Remark 	= trim(iconv("utf-8", "euc-kr",$_POST["Remark"]));				//비고
$DeviId 	= trim(iconv("utf-8", "euc-kr",$_POST["DeviId"]));					//단말기아이디
$AuthYn 	= trim(iconv("utf-8", "euc-kr",$_POST["AuthYn"]));				//인증여부
$Instmt 	= trim(iconv("utf-8", "euc-kr",$_POST["Instmt"]));						//할부개월수
$UserIp 	= iconv("utf-8", "euc-kr",$_SERVER["REMOTE_ADDR"]);			//회원 IP

/*신용카드(ISP)*/
$partial_mm 		= trim(iconv("utf-8", "euc-kr",$_POST["partial_mm"]));					//일반할부기간
$noIntMonth 		= trim(iconv("utf-8", "euc-kr",$_POST["noIntMonth"]));					//무이자할부기간
$KVP_CURRENCY 		= trim(iconv("utf-8", "euc-kr",$_POST["KVP_CURRENCY"]));	//KVP_통화코드
$KVP_CARDCODE 		= trim(iconv("utf-8", "euc-kr",$_POST["KVP_CARDCODE"]));	//KVP_카드사코드
$KVP_SESSIONKEY 	= iconv("utf-8", "euc-kr",$_POST["KVP_SESSIONKEY"]);		//KVP_SESSIONKEY
$KVP_ENCDATA 		= iconv("utf-8", "euc-kr",$_POST["KVP_ENCDATA"]);			//KVP_ENCDATA
$KVP_CONAME 		= trim(iconv("utf-8", "euc-kr",$_POST["KVP_CONAME"]));		//KVP_카드명
$KVP_NOINT 			= trim(iconv("utf-8", "euc-kr",$_POST["KVP_NOINT"]));				//KVP_무이자=1 일반=0
$KVP_QUOTA 			= trim(iconv("utf-8", "euc-kr",$_POST["KVP_QUOTA"]));		//KVP_할부개월

/*신용카드(안심)*/
$CardNo 			= trim(iconv("utf-8", "euc-kr",$_POST["CardNo"]));		//카드번호
$MPI_CAVV 			= iconv("utf-8", "euc-kr",$_POST["MPI_CAVV"]);		//MPI_CAVV
$MPI_ECI 			= iconv("utf-8", "euc-kr",$_POST["MPI_ECI"]);			//MPI_ECI
$MPI_MD64 			= iconv("utf-8", "euc-kr",$_POST["MPI_MD64"]);		//MPI_MD64

/*신용카드(일반)*/
$ExpMon 	= trim(iconv("utf-8", "euc-kr",$_POST["ExpMon"]));				//유효기간(월)
$ExpYear 	= trim(iconv("utf-8", "euc-kr",$_POST["ExpYear"]));				//유효기간(년)
$Passwd 	= trim(iconv("utf-8", "euc-kr",$_POST["Passwd"]));				//비밀번호
$SocId 		= trim(iconv("utf-8", "euc-kr",$_POST["SocId"]));					//주민등록번호/사업자등록번호

/*계좌이체사용*/
$ICHE_OUTBANKNAME	= trim(iconv("utf-8", "euc-kr",$_POST["ICHE_OUTBANKNAME"]));		//이체은행명
$ICHE_OUTACCTNO 	= trim(iconv("utf-8", "euc-kr",$_POST["ICHE_OUTACCTNO"]));				//이체계좌번호
$ICHE_OUTBANKMASTER = trim(iconv("utf-8", "euc-kr",$_POST["ICHE_OUTBANKMASTER"]));	//이체계좌소유주
$ICHE_AMOUNT 		= trim(iconv("utf-8", "euc-kr",$_POST["ICHE_AMOUNT"]));					//이체금액

/*핸드폰사용*/
$HP_SERVERINFO 		= trim(iconv("utf-8", "euc-kr",$_POST["HP_SERVERINFO"]));		//SERVER_INFO(핸드폰결제)
$HP_HANDPHONE 		= trim(iconv("utf-8", "euc-kr",$_POST["HP_HANDPHONE"]));		//HANDPHONE(핸드폰결제)
$HP_COMPANY 		= trim(iconv("utf-8", "euc-kr",$_POST["HP_COMPANY"]));			//COMPANY(핸드폰결제)
$HP_ID 				= trim(iconv("utf-8", "euc-kr",$_POST["HP_ID"]));								//HP_ID(핸드폰결제)
$HP_SUBID 			= trim(iconv("utf-8", "euc-kr",$_POST["HP_SUBID"]));					//HP_SUBID(핸드폰결제)
$HP_UNITType 		= trim(iconv("utf-8", "euc-kr",$_POST["HP_UNITType"]));				//HP_UNITType(핸드폰결제)
$HP_IDEN 			= trim(iconv("utf-8", "euc-kr",$_POST["HP_IDEN"]));							//HP_IDEN(핸드폰결제)
$HP_IPADDR 			= trim(iconv("utf-8", "euc-kr",$_POST["HP_IPADDR"]));					//HP_IPADDR(핸드폰결제)

/*ARS사용*/
$ARS_NAME 		= trim(iconv("utf-8", "euc-kr",$_POST["ARS_NAME"]));						//ARS_NAME(ARS결제)
$ARS_PHONE 		= trim(iconv("utf-8", "euc-kr",$_POST["ARS_PHONE"]));				//ARS_PHONE(ARS결제)

/*가상계좌사용*/
$VIRTUAL_CENTERCD	= trim(iconv("utf-8", "euc-kr",$_POST["VIRTUAL_CENTERCD"]));	//은행코드(가상계좌)
$VIRTUAL_DEPODT 	= trim(iconv("utf-8", "euc-kr",$_POST["VIRTUAL_DEPODT"]));		//입금예정일(가상계좌)
$ZuminCode 			= trim(iconv("utf-8", "euc-kr",$_POST["ZuminCode"]));				//주민번호(가상계좌)
$MallPage 			= trim(iconv("utf-8", "euc-kr",$_POST["MallPage"]));				//상점 입/출금 통보 페이지(가상계좌)
$VIRTUAL_NO 		= trim(iconv("utf-8", "euc-kr",$_POST["VIRTUAL_NO"]));			//가상계좌번호(가상계좌)

$mTId 				= trim(iconv("utf-8", "euc-kr",$_POST["mTId"]));

/*에스크로사용*/
$ES_SENDNO			= trim(iconv("utf-8", "euc-kr",$_POST["ES_SENDNO"]));			//에스크로전문번호

/*계좌이체(소켓) 결제 사용 변수*/
$ICHE_SOCKETYN			= trim(iconv("utf-8", "euc-kr",$_POST["ICHE_SOCKETYN"]));		//계좌이체(소켓) 사용 여부
$ICHE_POSMTID			= trim(iconv("utf-8", "euc-kr",$_POST["ICHE_POSMTID"]));			//계좌이체(소켓) 이용기관주문번호
$ICHE_FNBCMTID			= trim(iconv("utf-8", "euc-kr",$_POST["ICHE_FNBCMTID"]));		//계좌이체(소켓) FNBC거래번호
$ICHE_APTRTS			= trim(iconv("utf-8", "euc-kr",$_POST["ICHE_APTRTS"]));			//계좌이체(소켓) 이체 시각
$ICHE_REMARK1			= trim(iconv("utf-8", "euc-kr",$_POST["ICHE_REMARK1"]));			//계좌이체(소켓) 기타사항1
$ICHE_REMARK2			= trim(iconv("utf-8", "euc-kr",$_POST["ICHE_REMARK2"]));			//계좌이체(소켓) 기타사항2
$ICHE_ECWYN				= trim(iconv("utf-8", "euc-kr",$_POST["ICHE_ECWYN"]));			//계좌이체(소켓) 에스크로여부
$ICHE_ECWID				= trim(iconv("utf-8", "euc-kr",$_POST["ICHE_ECWID"]));			//계좌이체(소켓) 에스크로ID
$ICHE_ECWAMT1			= trim(iconv("utf-8", "euc-kr",$_POST["ICHE_ECWAMT1"]));			//계좌이체(소켓) 에스크로결제금액1
$ICHE_ECWAMT2			= trim(iconv("utf-8", "euc-kr",$_POST["ICHE_ECWAMT2"]));			//계좌이체(소켓) 에스크로결제금액2
$ICHE_CASHYN			= trim(iconv("utf-8", "euc-kr",$_POST["ICHE_CASHYN"]));			//계좌이체(소켓) 현금영수증발행여부
$ICHE_CASHGUBUN_CD		= trim(iconv("utf-8", "euc-kr",$_POST["ICHE_CASHGUBUN_CD"]));	//계좌이체(소켓) 현금영수증구분
$ICHE_CASHID_NO			= trim(iconv("utf-8", "euc-kr",$_POST["ICHE_CASHID_NO"]));		//계좌이체(소켓) 현금영수증신분확인번호
