...
function get_image($img, $width=0, $height=0)
{
    ...
    if (file_exists($full_img) && $img) 
    {
        ...
        $str = "<img id='$img' src='../data/item/$img' width='$width' height='$height' border='0'>";
    } 
    else 
    {
...
// 분류별 출력 
// 스킨파일번호, 1라인이미지수, 총라인수, 이미지폭, 이미지높이 , 분류번호
function display_category_check($no, $list_mod, $list_row, $img_width, $img_height)
{
	global $member, $g4;
    ...
    $sql = " select * from $g4[yc4_item_table] where it_use = '1' and main_rolling = '1' ";
    $sql .= " order by it_order, it_id desc limit $items ";
    $result = sql_query($sql);
    if (!mysql_num_rows($result)) {
        return false;
